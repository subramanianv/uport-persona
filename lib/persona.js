...
class Persona {
  ...
  load() {
    return new Promise((accept, reject) => {
      uportRegistry.getAttributes(this.registryAddress, this.address).then((tokens) => {
  ...
  setProfile(profile, privSignKey) {
    ...
    return uportRegistry.setAttributes(this.registryAddress, tokens, {from: this.address});
  }
  ...
  addClaim(token) {
    return new Promise((accept, reject) => {
      if (!Persona.isTokenValid(token)) {
        ...
        uportRegistry.setAttributes(this.registryAddress, this.tokenRecords, {from: this.address})
        .then(() => {
          accept();
        }).catch(reject);
      }
